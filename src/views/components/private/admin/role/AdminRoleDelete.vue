...
<script>
  ...
  import { deleteRoleApi } from '@/api/services'
  export default {
    ...
    methods: {
      ...
      async deleteItem () {
        ...
        const serviceResponse = await deleteRoleApi(this.roleComputed.id)
        if (serviceResponse.ok) {
          var index = this.rolesComputed.indexOf(this.roleComputed)
          this.rolesComputed.splice(index, 1)
          this.SET_ALERT({
